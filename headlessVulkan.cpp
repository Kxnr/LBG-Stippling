...
void HeadlessVulkan::CopyData(void* data, uint32_t bufferSize, VkBuffer& outputBuffer, VkDeviceMemory* outputMemory) {
  ...
  void *mapped;
  VK_CHECK_RESULT(vkMapMemory(device, *outputMemory, 0, bufferSize, 0, &mapped));
  memcpy(mapped, data, bufferSize);
  vkUnmapMemory(device, *outputMemory);
}

VkResult HeadlessVulkan::CreateBuffer(VkBufferUsageFlags usageFlags,  VkMemoryPropertyFlags memoryPropertyFlags, VkBuffer *buffer, VkDeviceMemory *memory, VkDeviceSize size, void *data) {
...
cimg_library::CImg<unsigned char> HeadlessVulkan::CopyImage() {
  ...
  for (int32_t y = 0; y < height; y++) {
    ...
    for (int32_t x = 0; x < width; x++) {
      ...
      // std::cout << std::hex << *row << " " << ((*row >> 16) & 0xff) << " " << ((*row >> 8) & 0xff) << " " << ((*row) & 0xff)<< std::endl;

      out(x, y, 0) = (unsigned char)((*row >> 0) & 0xff);
      out(x, y, 1) = (unsigned char)((*row >> 8) & 0xff); 
      out(x, y, 2) = (unsigned char)((*row >> 16) & 0xff);
